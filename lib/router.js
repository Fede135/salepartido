...
Router.route('/gestionarreserva/modificarReserva', {name:'modificarReserva'});
...
var inicio= function() {
    if (Meteor.userId()) {
      ...
      if (Roles.userIsInRole(Meteor.userId(),'owner')) {
        ...
        if (recinto) {
          //if (Canchas.findOne({recintoId: recinto_id})){  queda comentado porque falta ver que hacer con el tincho y las rutas de cargar cancha. la idea aca es que sino ha cargado ninguna cancha para su recinto al momento de ingresar lo haga.
