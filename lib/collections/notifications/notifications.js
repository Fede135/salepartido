...
cancelacionReservaPlayersNotificationDeleteOwner = function (partidoId, arrayInvitados) {
  console.log("notif",partidoId, arrayInvitados);
  var monthNames = [
    "Enero", "Febrero", "Marzo",
    "Abril", "Mayo", "Junio", "Julio",
    "Agosto", "Septiembre", "Octubre",
...
cancelacionReservaPlayersNotificationDeleteOwner2 = function (partidoId, idPlayer) {
  var monthNames = [
    "Enero", "Febrero", "Marzo",
    "Abril", "Mayo", "Junio", "Julio",
    "Agosto", "Septiembre", "Octubre",
    ...
    var partido = Partido.findOne({_id: partidoId});
    var host = Meteor.users.findOne({_id:partido.hostId});
    if(Roles.userIsInRole(idPlayer, 'confirmado', partidoId)) {
      var texto = "Se canceló el partido de "+host.profile.name+" al que estabas confirmado.";
    } else if (Roles.userIsInRole(idPlayer, 'invitado', partidoId)) {
      var texto = "Se canceló el partido de "+host.profile.name+" al que estabas invitado.";
    } else if (Roles.userIsInRole(idPlayer, 'suplente', partidoId)) {
      var texto = "Se canceló el partido de "+host.profile.name+" en el cual eras suplente.";
    };
    Notifications.insert({
      toUserId: idPlayer,
...
eliminadoPorHostNotification = function (partidoId, jugadorId) {
  var monthNames = [
    "Enero", "Febrero", "Marzo",
    "Abril", "Mayo", "Junio", "Julio",
    "Agosto", "Septiembre", "Octubre",
    ...
    var hostName = Meteor.users.findOne({_id:partido.hostId}, {fields: {'profile.name': 1}});
      ...
      commentId: partidoId,
      link: Router.routes.confirmarPartido.path({_id: partidoId}),
      text: "El anfitrión "+hostName.profile.name+" te eliminó del equipo, donde habías confirmado tu asistencia.",
      read: false
    });     
}
